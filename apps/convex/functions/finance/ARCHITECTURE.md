...
**functions/finance/roles.ts** — Fund role (finance_admin / manager / cardholder) lifecycle. Exports `grantFundRole` and `revokeFundRole` (mutations: **COMMUNITY finance access only** since ADR-033 Phase 3 — the primary admin or an explicit `communityFinanceRoles` grant, and the caller may be entirely outside the fund's group; see "Groups operate, community finance administers" below. `revokeFundRole` is deliberately NOT flag-gated, and no longer carries a "last finance_admin" guard), `listFundRoles`, and `getMyFundRole` (queries: both flag-gated; leaders and managers still read the roster).
...
**functions/finance/cards.ts** — Group-fund virtual cards (ADR-032 §3 Phase 3). Exports `listFundCards` / `getCardDetail` (cardholder+/leader/admin-gated reads; `getCardDetail` includes the card's last 20 `card_charge` expenses), `createFundCard` (mutation: **a COMMUNITY finance admin** issues a card to a cardholder+ holder — see "Groups operate, community finance administers"; rejects a non-`active` fund, a fund with no `increaseAccountId` (Increase) or no live connection (BYO), a community whose `communityFinance.onboardingStatus` isn't `"live"`, or a second live card on a fund whose provider declares `maxCardsPerFund: 1`; inserts a `"pending"` row, audits `card.created`, then schedules `provisionCard`, an internalAction that calls Increase's `createCard` and records the result via `recordCardProvisioned`/`recordCardProvisionFailed` — same provider-write-then-record shape as onboarding.ts's `provisionProviders`), `setCardLimit` (mutation: finance_admin changes or clears a live card's limit — same readiness gates as `createFundCard`, i.e. an `active` fund and a card that is `active`/`disabled`; a `canceled` card and a frozen fund are both refused, since changing a cap is mostly a spending-power grant. It returns `{ status: "pending", requested* }`, never the limit as if applied), `setCardFrozen` (mutation: the holder can self-freeze, but only a finance_admin can unfreeze — a compromised holder must not be able to re-enable their own frozen card), and `cancelCard` (mutation: finance_admin-only, irreversible). The lifecycle mutations schedule the shared `applyCardStatus` internalAction (`updateCardStatus` at Increase, then `recordCardStatus` persists whatever status Increase actually confirmed); `setCardLimit` schedules the equivalent `applyCardLimit`/`recordCardLimit` pair. The settlement side — turning a confirmed card swipe into an expense — is `webhooks.ts`'s `recordCardSettlement`, not this file.

ADR-033 Phase 3 adds the MANAGED-LIMIT trio to this module: `syncManagedCardLimit` (internalAction — recomputes a managed card's lifetime cap from its fund's ledger and pushes it), `getManagedCardSyncTarget` / `recordManagedCardLimit` (its query/recorder pair, same provider-write-then-record shape as `applyCardLimit`), and `listManagedCards` (the hourly backstop's fan-out). On a managed card, `setCardLimit` sets the admin's **manual cap** rather than the limit — see "Managed card limits" below.

**The card provider is pluggable** (ADR-033). `cards.ts` no longer names Increase: its three internalActions resolve an adapter via `getCardProvider(ctx, communityId)` (**lib/finance/cardProviders/**) and call `createCard` / `setCardState` / `setSpendLimit` on it. `communityFinance.cardProvider` picks the issuer; an absent value resolves to `"increase"` when the community already has Increase ids (every community today), and to a clear throw otherwise. The **Increase adapter** (`cardProviders/increase.ts`) wraps `lib/finance/increase.ts` and owns the Increase-*semantic* pieces that used to sit in cardPolicy — the period→interval map and `cardLimitWindowStart`'s UTC window math; `validateCardLimit` stays shared. **Status vocabulary is split on purpose**: the adapter speaks normalized `pending|active|paused|closed|failed`, but `cards.status` still stores the legacy Increase strings (`active`/`disabled`/`canceled`) because mobile's `CardStatus` union is pinned to them — ADR-033 Phase 1 migrates the column. Credentials for bring-your-own providers live in `cardProviderConnections` under AES-256-GCM envelope encryption (`lib/finance/credentialCrypto.ts`, `CREDENTIALS_MASTER_KEY`), never plaintext.

**Spend limits are provider-enforced, not advisory.** `spendLimitCents`/`limitPeriod` are translated by the **card adapter** (for Increase, `lib/finance/cardProviders/increase.ts`) into Increase's `authorization_controls.usage.multi_use.spending_limits` and sent at card creation and on every change, so Increase declines an over-limit authorization without calling us ([Increase: Launching a card program](https://increase.com/documentation/launch-a-card-program) — "enforced at authorization time without a round trip to your server"). Increase's own reset semantics apply and are UTC: `week` → `per_week` (resets Mondays at midnight UTC), `month` → `per_month` (the 1st, midnight UTC), `charge` → `per_transaction`. The stored pair is a **mirror** of the bank's copy, only ever written after Increase confirms the change; `webhooks.ts` compares settlements against it and audits `card.limit_exceeded` when the two disagree.
...
- Action strings (dot-namespaced):
  ...
  - `role.granted` — Fund-scoped finance role assigned to a user
  - `role.revoked` — Fund-scoped finance role soft-deleted
  - `community_role.granted` — Community-wide financial-controls access granted by the primary admin (ADR-033 §5)
  - `community_role.revoked` — Community-wide financial-controls access soft-deleted
...
**Community admin override** (ADR-032 §4) — Community admins always pass requireFundRole checks; admins can do anything on any *fund* without an explicit grant. Mirrors requireCommunityAdmin + requireGroupLeaderOrCommunityAdmin patterns. **Unchanged by ADR-033** — see below.

**Community-level financial controls** (ADR-033 §5, lib/finance/communityFinanceAccess.ts) — "can run the community" and "can run the community's *money*" used to be the same check. They aren't any more. Community-wide finance surfaces gate on `canManageCommunityFinance` = **primary admin (implicit) OR an active `communityFinanceRoles` row**, granted and revoked by the primary admin only (`functions/finance/communityRoles.ts`). A plain community admin (roles === 3) with no grant no longer reaches finance onboarding, the Stripe verification redirect, `enableGroupGiving`, or the community-wide signal in `getMyFundRole` (renamed `isCommunityAdmin` → `canManageCommunityFinance`). Being able to add a channel should not carry the ability to hand a vendor a spending credential. The primary admin's access is *implicit* — no seeded row — so there was nothing to migrate and no revoke can lock a community out of its own finances. Fund-level access (`requireFundRole` / `fundRoles`) is deliberately untouched.

**Groups operate, community finance administers** (ADR-033 Phase 3) — the three fund surfaces that are APPOINTMENTS rather than operations now gate on `requireCommunityFinanceAccess`: `createFundCard`, `grantFundRole`, `revokeFundRole`.

*What this replaced, and why the old guard wasn't enough.* ADR-032 §4 gave group leaders a narrow role-grant power on their own fund (`requireFundRoleOrGroupLeader` ignores `minRole` by design, so a leader with no fund role passes a `"finance_admin"` gate). That was a one-tap privilege ladder — leader → `grantFundRole(self, "finance_admin")` → `createFundCard(self, …)` → spend the fund alone — and it was closed by having `requireFundRole` report **which** path allowed the call (`FundAccessVia`) and refusing a self-targeted grant from `"group_leader"`. But that only ever removed one rung: two leaders of the same group could grant each other. And at a bring-your-own provider the card they end up with spends the CHURCH's pooled account, not a per-fund one the bank keeps separate — so the whole ladder ran inside one group over money that isn't only theirs.

*What still works exactly as before.* Everything operational: a cardholder uses their card; a manager approves and denies reimbursements; a leader reads the fund's roster, balance and transactions; `setCardFrozen` / `cancelCard` keep their fund gate **and** their kill-switch exemption, so de-escalation never needs a phone call to someone outside the group. `requireFundRole` / `requireFundRoleOrGroupLeader` and the `FundAccessVia` return are unchanged and still back every one of those.

*`enableGroupGiving` still seeds the first roles* — it grants current leaders finance_admin on the new fund, which is what stops the move from leaving a fund with nobody able to approve anything.

Residual, not closed by code: a leader can still be granted `manager` alongside a second account **they** control, which defeats the two-approver threshold — the appointment now costs a community finance admin's cooperation, which raises the bar without closing it. See the sock-puppet entry in `docs/finance/COMPLIANCE.md`.
...
**`getGivingContext` fails open on membership too, not just on the flag** — it is the only read that answers "should this surface show a fund at all?", and it runs on `GroupInfoScreen` / `GroupDetailScreen`: routes a **non-member** legitimately reaches from a share link or Explore. Convex re-throws a query's server error synchronously during render, so `assertCanViewFund`'s throw landed on the app's root `ErrorBoundary` — a full crash screen in place of the join CTA, for every non-member of any group with a fund. It now uses the non-throwing `canViewFund` and returns `null`, matching what its sibling reads on those screens already do (`countGroupJoinRequests` → `0`, `getGroupNotifications` → defaults). "You can't see it" and "there isn't one" are the same answer to that question, and nothing about the fund leaks either way. This is scoped to this one query: `getFundOverview` and `prepareDonationIntent` still `assertCanViewFund`, because by the time they run the caller has committed to showing fund detail or moving money, and a refusal there should be loud. Both mobile screens additionally pass `"skip"` for a non-member so the query never fires — belt and braces, and one fewer subscription.

**Receipt provenance** — an `r2:<key>` string is not self-authenticating, and receipt URLs are shown to every manager+ viewer (`listExpenses`) and card viewer (`getCardDetail`). An `uploadGrants` row (storagePath → userId) records who each key was minted for, and both writers — `submitExpense` and `attachExpenseReceipt` — refuse a receipt whose grant is missing or belongs to someone else, through the shared `assertReceiptOwnedBy`.

**Managed card limits** (ADR-033 Phase 3, lib/finance/managedCardLimit.ts) — at a provider with no `hardFundIsolation`, a card's cap is COMPUTED from its fund's ledger rather than typed by an admin:

```
L = lifetime CREDITS − non-card DEBITS      →  remaining at the provider == the fund's balance
```

`card_capture` debits are excluded because the provider already counts that spend against its lifetime cap. Two preconditions, both enforced: **one live card per fund** (the cap is per card; two would each carry the fund's whole allowance) and every card charge reaching the ledger. The card stores the amount in `spendLimitCents` with **no** `limitPeriod` — the window is lifetime, which the column can't express — and declares itself through `controls: { managedLimit: true, manualCapCents? }`. The resync is scheduled from ONE seam, inside `postLedgerEntry`, so any future path that posts an entry gets it without remembering to; it skips `card_capture` (which changes nothing) and recomputes from scratch every run, which is what makes it idempotent. A failed push is left for `finance-card-txn-poll`'s hourly resync, which runs AFTER the import so it computes against a current ledger.

**Receipt forwarding** (ADR-033 Phase 3) — `attachExpenseReceipt` is the write behind the "No receipt" badge (a card charge is created by the settlement webhook before anyone has the paper, so ADR-032's nudge flow had no write path). Where the card's issuer declares `capabilities.receiptForwarding` — BILL alone — it schedules `forwardExpenseReceipt`, which sends the image on via BILL's documented three-step upload. Best-effort by contract: the receipt is already durable here and the church's books are complete without the copy at the issuer, so a failure is an audit row (`expense.receipt_forward_failed`) and never undoes the attachment. Dedupe is a staleness check, not a marker column: one run per attachment, and the action stops unless `receiptKey` is still the key it was scheduled with.
...
- **finance-roles.test.ts** — Tests the ADR-033 Phase 3 gate: a community finance admin grants and revokes from OUTSIDE the fund's group; a leader, a fund finance_admin and a plain community admin are all refused (including the rung the old self-grant guard left open — a leader granting a *willing second leader*); the target must still be an active member; revoke stays reachable with the kill switch off while grant doesn't; and the operational surfaces (roster read, `getMyFundRole`'s two separate community signals) are unchanged
- **finance-managed-card-limit.test.ts** — Tests the managed lifetime cap: the formula and its worked example (500 in, 100 reimbursed, 250 swiped → cap 400, 150 left, balance 150), the zero floor, monotonic raise on a donation and lower on a non-card debit while a card charge moves nothing, repeat-sync no-ops, the one-card-per-fund refusal (including a `pending` card holding the slot), manual-cap interplay in both directions, a provider refusal leaving the mirror honest, and the hourly poll re-applying a cap whose live sync failed
- **finance-receipt-forwarding.test.ts** — Tests `attachExpenseReceipt` (cardholder or manager+, provenance, card charges only) and the forward: the capability boundary in both directions, BILL's three-step upload in order with the url reused in step 3 and no `apiToken` sent to the pre-signed host, the replaced-receipt staleness guard, a format BILL can't open refused before upload, and a provider failure never undoing the receipt
- **finance-byoc-capabilities.test.ts** — Pins the name-keyed client capability table against every adapter's own `capabilities`, so the copy that exists to avoid decrypting a credential can't drift from the truth
...
- **What card controls still DON'T do.** Increase enforces the amount-per-interval
  ...
  requirement — a charge with no receipt sits `"pending"` and is visible, and
  since ADR-033 Phase 3 the cardholder can finally ATTACH one
  (`attachExpenseReceipt`), but nothing chases them and nothing blocks the card.
  All of those need either
...
- **The managed limit is not a real-time authorization decision.** It is a
  cumulative cap the provider enforces on its own, so two charges racing before
  either settles are judged against the number Privacy currently holds rather
  than against the fund. That is the conservative direction and it is not the
  same guarantee as a bank refusing to let a card touch another fund's money —
  `hardFundIsolation: false` still means what it says. The managed cap makes
  over-spend bounded and self-correcting rather than impossible.

- **BILL has no managed-limit equivalent.** Its per-fund container is a BUDGET,
  which is a spending policy rather than segregated money, so
  `managedFundLimit` is false there and the budget's own cap — a number the
  church set in BILL, which Togather does not manage — is the only fund
  boundary. `requiresSpendLimit` still forces a per-card cap on top. A church on
  BILL is relying on two numbers, one of which lives outside this system.
