...
- Action strings (dot-namespaced):
  ...
  - `card.limit_updated` — A finance_admin changed or cleared a card's spend limit (`from*`/`to*` values in details); logged at request time, before Increase confirms
...
- **What card controls still DON'T do.** Increase enforces the amount-per-interval
  ...
  **The rule this seam exists to protect:** no card surface may claim a control
  ...
  a "Require receipts — On" toggle backed by nothing at all.
