...
// ============================================================================
// TODO: Remove these once all clients are updated to use mutations

/**
 * @deprecated Use startPlanningCenterAuthMutation instead
 * Start Planning Center OAuth flow
 * Generates authorization URL with state token
 */
export const startPlanningCenterAuth = action({
  args: {
    token: v.string(),
    communityId: v.id("communities"),
    redirectUri: v.optional(v.string()),
  ...
  handler: async (ctx, args) => {
    // Verify auth and admin status via internal mutation
    const result = await ctx.runMutation(internal.functions.integrations.verifyAdminAccess, {
      token: args.token,
      communityId: args.communityId,
    });
    ...
    // Generate signed state token for CSRF protection
    // Note: We use the user's legacy ID if available for OAuth state
    const stateToken = await signOAuthState(
      result.userId,
      result.communityLegacyId || 0, // Legacy community ID for OAuth state
      redirectUri
    );
...
/**
 * @deprecated Use completePlanningCenterAuthMutation instead
 * Handle OAuth callback - exchange code for tokens
 */
export const completePlanningCenterAuth = action({
  args: {
    token: v.string(),
    communityId: v.id("communities"),
    code: v.string(),
  ...
  handler: async (ctx, args) => {
    // Verify auth
    const authResult = await ctx.runMutation(internal.functions.integrations.verifyAdminAccess, {
      token: args.token,
      communityId: args.communityId,
    });
...
/**
 * @deprecated Use disconnectPlanningCenterMutation instead
 * Disconnect Planning Center integration
 */
export const disconnectPlanningCenter = action({
  args: {
    token: v.string(),
    communityId: v.id("communities"),
  },
  handler: async (ctx, args) => {
    ...
    await ctx.runMutation(internal.functions.integrations.clearPlanningCenterIntegration, {
      communityId: args.communityId,
