...
export const notifySuperAdminsOfProposal = internalAction({
  ...
  handler: async (ctx, args): Promise<{ success: boolean; error?: string; sent?: number }> => {
    try {
      // Get the proposal
      const proposal = await ctx.runQuery(internal.functions.notifications.proposalNotifications.getProposal, {
      ...
      const superAdminIds: Id<"users">[] = await ctx.runQuery(
        internal.functions.notifications.proposalNotifications.getSuperAdminUsers,
      );
