  ...
  handler: async (ctx, args) => {
    ...
    for (const batchRow of args.batchRows) {
      ...
      if (batchRow.notes) {
        ...
        if (existingCsvNote) {
          // Strip existing tag line, append new note, re-add tag at bottom
 ...
 * text is on top and the [csv import] tag + timestamp is at the bottom.
 *
 * Run via: npx convex run functions/memberFollowups:backfillCsvImportNotes
 */
export const backfillCsvImportNotes = mutation({
  ...
  handler: async (ctx) => {
    ...
    for (const note of csvNotes) {
      ...
      // First line is always "[csv import]", remaining lines are "timestamp - note text"
