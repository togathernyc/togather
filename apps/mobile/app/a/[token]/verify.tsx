...
import { api, convexVanilla } from "@/services/api/convex";
...
export default function AvailabilityVerifyScreen() {
    ...
    try {
      ...
      await convexVanilla.mutation(
        api.functions.scheduling.publicAvailability.submitAvailabilityForRequest,
        {
          token: authToken,
          publicToken: token,
          responses: JSON.parse(responses) as AvailabilityResponse[],
        }
      );
      return true;
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (userExists && userHasVerifiedPhone) {
        // Case 1: Existing verified user - verify OTP, get tokens, submit availability
        ...
        // Persist tokens + sync AuthProvider state so downstream calls (like availability submit) can authenticate
        ...
        // This ensures tabs and other features work correctly after auth
        try {
