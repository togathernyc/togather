...
**Receipt Provenance** — A reimbursement's `receiptKey` is an R2 object path, and receipt URLs are surfaced to every manager+ viewer of a fund. `submitExpense` previously accepted any `r2:`-prefixed string, so a submitter could attach another member's receipt image as proof of purchase. `functions/uploads.ts` now records an `uploadGrants` row (storage path → uploading user) at the moment it mints a presigned upload URL, and `submitExpense` refuses a receipt whose grant is missing or belongs to someone else. OPEN ITEM: the same key may still be attached to more than one expense by its rightful owner (duplicate-submission control, not an impersonation one) — a candidate for the fraud runbook.
