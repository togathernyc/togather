...
**No Self-Escalation via the Leader Carve-Out** — ADR-032 §4 lets an active group leader grant finance roles on their own group's fund without holding one themselves; that carve-out (`requireFundRoleOrGroupLeader`, `lib/helpers.ts`) is how a fund gets its first treasurer. It deliberately ignores the required role level, which previously made it a single-actor privilege ladder: a leader could grant **themselves** `finance_admin`, then issue themselves a virtual card (`createFundCard` gates on `finance_admin`) and spend the fund alone, with no second human anywhere in the chain. Closed by making the permission helpers report *which* access path allowed a call (`FundAccessVia`: `community_admin` / `fund_role` / `group_leader`) and having `grantFundRole` (`functions/finance/roles.ts`) refuse a self-targeted grant when the answer is `group_leader`. The strong paths resolve first, so a community admin who also leads the group is unaffected, and granting to *another* member — the ADR's sanctioned bootstrap — still works. Covered by `finance-roles.test.ts`, including an assertion that the leader still cannot issue themselves a card.

**Collusion / Sock-Puppet Residual Risk** — The two-approver threshold is a control on *distinct approvals*, not on *distinct trust*. It assumes the two approvers are independent humans acting in good faith. Self-escalation is now blocked (above), so a single actor can no longer promote *themselves* — but the assumption is still not structurally enforced: an active group leader can grant `manager` to a **second account they also control** and approve with it, and community admins bypass fund-role checks entirely (`requireFundRole`'s admin override), so an admin can both submit and effectively steer approval on any fund. Both are collusion between two identities, which no permission check can distinguish from two real people. The threshold therefore remains trust-scoped to leader/admin integrity, not a guarantee against one bad actor operating two accounts. Accepted residual risk, flagged for the fraud/ops runbook (OPEN ITEM: define detection heuristics, e.g. approver pairs sharing a device/IP/payout destination, or grant chains where the grantor and grantee never act independently, for ops to monitor).

**Receipt Provenance** — A reimbursement's `receiptKey` is an R2 object path, and receipt URLs are surfaced to every manager+ viewer of a fund. `submitExpense` previously accepted any `r2:`-prefixed string, so a submitter could attach another member's receipt image as proof of purchase. Both paths that hand out an `r2:` key now record an `uploadGrants` row (storage path → uploading user) — `functions/uploads.ts` at presign time, and `lib/r2.ts`'s `putR2Object` when a server-side upload names the member it's for — and `submitExpense` refuses a receipt whose grant is missing or belongs to someone else. The write is best-effort on both paths (a finance check must not be able to break every upload in the product); the gate is the read, so a lost row costs a re-attach, never a false accept. OPEN ITEM: the same key may still be attached to more than one expense by its rightful owner (duplicate-submission control, not an impersonation one) — a candidate for the fraud runbook.
...
**Last Updated**: 2026-07-31 | **Status**: Ready for counsel & underwriting review | **Next Steps**: Resolve open items in consultation with legal counsel and Increase partner
